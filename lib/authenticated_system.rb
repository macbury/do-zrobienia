module AuthenticatedSystem
  protected
    ...
    def self.included(base)
      base.send :helper_method, :current_uzytkownik, :logged_in?
    end

    # Called from #current_uzytkownik.  First attempt to login by the uzytkownik id stored in the session.
    def login_from_session
      self.current_uzytkownik = Uzytkownik.find_by_id(session[:uzytkownik_id], :select => "`uzytkownicy`.`id`, `uzytkownicy`.`login`") if session[:uzytkownik_id]
    end
    ...
    def login_from_cookie
      uzytkownik = cookies[:auth_token] && Uzytkownik.find_by_remember_token(cookies[:auth_token], :select => "`uzytkownicy`.`id`, `uzytkownicy`.`login`, `uzytkownicy`.`remember_token_expires_at`, `uzytkownicy`.`remember_token`")
